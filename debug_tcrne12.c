...
L(d[++t]!=0)C(dt,H);
F(reset(w[f%2],17408))w[f%2].A=U+0*layer_sort(w[f%2],-1)*(++s+0*layer_sort(w[1-f%2],1));
}B(c<252);
...
B(s<4);
sm(5);
}D main(){F((t=-1)_(video_mode=3+d3d_antialias=9*video_screen=1)_(w0=view_create(-1))_(w1=camera))m;
...
E(G,E(v,MM(V(Z(Y.h),N(Y.h),0),50+abs(50*Z(O*2)))));
E(w0.x,kk(G,Y.x));
...
L(s<3){G+=T*30+0*path_spline(M,w1.x,G);
...
}D ss(){F(Gf=M.a)L(s<3)F(m _(Gf=minv(Gf+T*.8,57))>=57)Gf=0*W(-2);
}D sv(){B(f<1);
L(s<2){F((G+=T)>5)G-=5+0*e(pe,15,M.x,d[0].a);
...
}F((s<1||s>3)_ c>9_(G+=T)>3)G%=3+0*e(ei,10,M.a,d[0].a)*e(es,6,M.S[1+s],M.S[8+s]);
m;
}}D re(){L(1+m)M.h+=T*G;
}D mi(){L(1+m){M.x=G+(c>9)*16*N(O*(1+(f>3))*9+M.a)+0*(M.z=cycle(M.z+uv,-272,624));
}}
//comment your code!