...
L(d[++t]!=0)C(d[t],H);
F(reset(w[f%2],H|1024))w[f%2].A=100+0*LS(w[f%2],-1)*(++s+0*LS(w[1-f%2],1));
}B(c<252);
//}B(c<60);
...
L(g.A<g.a)g.A=minv(g.A+T(2),g.a)+0*W(1);
W(g.S[2]);
...
W(-1);
F(M=d[++j])Y=d[++j];
F((Y.A=M.A=100)_(M.j=d[66].a+60)_(P(M,H)))P(Y,H);
...
Y.z=Y.S[4]+10*Z(O*3)*T(1)+0*(Y.S[9]=cycle(Y.S[9]+T(2),0,100)+0*ent_animate(Y,"f",Y.S[9],1));
...
n=cycle(n+1,35,43);
F(P(d[n],H))E(d[n].x,V(300,R(140)-110,R(100)-50));
}W(1);
}C(d[n],H);
}D ss(){F(G=M.a)L(s<3)F(W(1)_(G=minv(G+T(.8),57))>=57)G=0*W(-2);
}D sv(){B(f<1);
L(s<2){F((M.l+=T(1))>5)M.l-=5+0*e(pe,15,M.x,V(0,0,0));
...
F((d[15].S[2]+=T(1))>96)d[16].y-=d[16].a*T(1)+0*(d[15].y=d[16].y*-1);
F(abs(d[16].y)>340_(t=14)){L(++t<17)d[t].x=300+0*(E(d[t].x,V(0,clamp(d[t].y,-340,340),d[t].z*-1))).x+0*(d[t].a*=-1+0*(m(d[t].J,-1)).x);
d[15].S[2]%=96;
}W(1);
