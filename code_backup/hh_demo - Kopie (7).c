...
}I(ee)F((aa=pa)q,2100480)))p.A=25+R(25)+0*(k(p.vel_x,U(V(R(2)-1,R(2)-1,R(20)),R(50)))).x*(E(p.blue,V(20,110,150))).x;
}I(pe)F((aa=mw)q,2304))&&(p.lifespan=60)&&(p.size=2))p.A=45+0*(E(p.blue,V(180,80,62))).x*(E(p.vel_x,V(-1+R(2),-1+R(2),3.5+R(1.5)))).x;
}I(es)F(!(aa=0)q,2099456))&&(p.size=4)&&(p.vel_z*=8+R(4)))p.A=20+R(10)+0*(E(p.blue,V(255,255,255))).x*(k(p.x,V(R(220)-110,R(1400)-700,0))).x;
}I(ei)F((aa=pa)q,STREAK|BRIGHT))&&(p.size=2))p.A=25+R(25)+0*(k(p.vel_x,U(V(R(2)-1,R(2)-1,R(2)-1),R(150)))).x*(E(p.blue,V(255,255,255))).x;
}D fa(){W(1);
F((w[0].A=100)&&(w[0].fog_end=w[1].fog_end=1100)&&((fog_color=d[3].S[5])>-1)&&(t=9))L(d[++t]!=0)F(d[t].A=d[t].l)P(d[t],H);
L(f<4){B(c<d[3].S[f+1]);
//L(f<4){B(c<10+10*f);
t=9+5*f++;
L(d[++t]!=0)P(d[t],H);
F((set(w[f%2],1024))&&(set(w[1-f%2],H)))w[1-f%2].dd=1000;
...
B(c<153);
F(M=d[j])Y=d[++j];
F((P(M,H))&&(M.j=-45)&&(M.A=Y.A=100))P(Y,H);
L(j<80){F(M.l==0){d[66].S[1]=M.j;
L(M.A>0)Y.A=X(M.A,-T(4))+0*(Y.j=M.j+=T(0.5))*W(1);
F(!C(M,H))C(Y,H);
W(-1);
F(M=d[++j])Y=d[++j];
F((Y.A=M.A=100)&&(M.j=d[66].S[1]+60)&&(P(M,H)))P(Y,H);
...
F((t=-1)&&(d3d_antialias=9)&&(fps_max=100)&&(GetWindowRect(GetDesktopWindow(),&n)!=0)&&(w[0]=view_create(-1))&&(w[1]=camera))video_set(n.right,n.bottom,32,1);
...
F(Y=M.p)d[9]=Y.p;
L(s<4){F((d[9].h-=T(10))&&(M.h+=T(9))&&(Y.h+=T(5)))M.ambient=70*abs(Z(O*30))+0*(Y.ambient=70*abs(N(O*30)));
F(f<4&&s>2)d[25].A=d[25].l-20*abs(N(O*30));
E(M.l,E(v,m(V(Z(Y.h),N(Y.h),0),50+abs(50*Z(O*2)))));
w[0].z=Y.z-abs(450*Z(O*6))+0*(E(w[0].x,k(M.l,Y.x))).x;
M.z=0*(E(M.x,k(M.l,v))).x*(E(w[0].h,V(w[0].h-T(7),-90+35*N(O*5),0))).x*W(1);
...
d[18].y=300*N(O)+0*(d[17].y=300*Z(O*1.5))*(E(w[0].h,V(w[0].h+T(3)*(0.2+abs(N(O))),-15-5*N(O*2),15*Z(O*6)))).x*(k(vec_rotate(E(w[0].x,V(-400-250*N(O*5),0,70)),w[0].h),M.x)).x;
F((d[15].S[2]+=T(1))>96)d[16].y-=d[16].S[1]*T(1)+0*(d[15].y=d[16].y*-1);
F(abs(d[16].y)>340&&(t=14)){L(++t<17)d[t].x=300+0*(E(d[t].x,V(0,clamp(d[t].y,-340,340),d[t].z*-1))).x+0*(d[t].S[1]*=-1+0*(m(d[t].J,-1)).x);
d[15].S[2]%=96;
}W(1);
}}D set_intro(){F(0*W(snd_play(snd_create("n.ogg"),100,0)!=0)==0)sh();	
...
F((s==0||s==4)&&c>9&&(M.S[3]+=T(1))>3)M.S[3]%=3+0*e(ei,10,M.l,V(0,0,0))*e(es,6,M.S[s],M.S[7+s]);
W(1);
}}