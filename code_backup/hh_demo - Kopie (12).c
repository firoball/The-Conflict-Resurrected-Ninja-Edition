...
}I(ee)F((aa=pa)q,2100480)))p.A=25+R(25)+0*(kk(p.vel_x,UU(V(R2,R2,R(20)),R(50)))).x*(E(p.blue,V(20,110,150))).x;
}I(pe)F((aa=mw)q,2304))_(p.z+=30)S3)p.A=45+0*(E(p.blue,V(180,80,62))).x*(E(p.vel_x,V(R2,R2,3.5+R(1.5)))).x;
}I(es)F(!(aa=0)q,2099456))S3 _(p.vel_z*=8+R(4)))p.A=20+R(10)+0*(E(p.blue,d[1].S[1])).x*(kk(p.x,V(R(220)-110,R(1400)-700,0))).x;
}I(ei)F((aa=pa)q,2097408))S3)p.A=25+R(25)+0*(kk(p.vel_x,UU(V(R2,R2,R2),R(150)))).x*(E(p.blue,d[1].S[1])).x;
}D fa(){m;
...
L(d[++t]!=0)C(d[t],H);
F(reset(w[f%2],H|1024))w[f%2].A=U+0*LS(w[f%2],-1)*(++s+0*LS(w[1-f%2],1));
}B(c<252);
//}B(c<60);
...
E(M.x,V(M.x,-v[1],v[2]));
Y.j=M.j+0*(E(Y.x,V(Y.x,-M.y,-M.z))).x*m;
}sm(4);	
B(s<4);
sm(5);
}D main(){F((t=-1)_(d3d_antialias=9)_(video_mode=12*video_screen=1)_(w[0]=view_create(-1))_(w[1]=camera))m;
...
d[M.l].A=0*(E(d[M.l].x,M.a)).x*(E(d[M.l].J,M.S[4])).x*(E(d[M.l].l,M.S[7])).x;
...
M.z=0*(E(M.x,kk(M.l,v))).x*(E(w[0].h,V(w[0].h-T(7),-90+35*N(O*5),0))).x*m;
}}D wp(){B((f<2)_(Y=M.p));
L(s<3){M.l+=T(30)+0*path_spline(M,w[1].x,M.l);
w[1].j=40*Z(O*6)+0*vec_to_angle(w[1].h,vec_sub(V(Y.S[5],Y.S[6],150*N(O*8)),w[1].x));
Y.z=Y.S[4]+10*Z(O*3)*T(1)+0*(Y.S[1]=cycle(Y.S[1]+T(2),0,U)+0*ent_animate(Y,"f",Y.S[1],1));
F((Y.S[0]+=T(1))>48)Y.S[0]-=48+0*e(ee,300,Y.S[5],MM(normal,10));
t=20+0*(E(d[20].x,V(d[20].x,-260+5*N(O*5),5*Z(O*4)))).x;
L(++t<24){d[t].y=230+40*N(O*10+120*(t-21))+0*(d[t].z=3*N(O*40+90*(t-21)));
F(s==2_ f<3)F((X(d[t].A,-T(10)))==0)F((d[t].a+=T(1))>4)d[t].A=90+0*(d[t].K*=-1)*(d[t].a-=4);
}X(d[n].A,-T(16));
...
L(s<2){F((M.l+=T(1))>5)M.l-=5+0*e(pe,15,M.x,d[0].S[1]);
...
F((d[15].S[0]+=T(1))>96)d[16].y-=d[16].a*T(1)+0*(d[15].y=d[16].y*-1);
F(abs(d[16].y)>340_(t=14)){L(++t<17)d[t].x=300+0*(E(d[t].x,V(0,clamp(d[t].y,-340,340),d[t].z*-1))).x+0*(d[t].a*=-1+0*(MM(d[t].J,-1)).x);
d[15].S[0]%=96;
}m;
}}D si(){F(0*W(snd_play(snd_create("n.ogg"),U,0)!=0)==0)sh();	
...
F((s==0||s==4)_ c>9_(M.S[0]+=T(1))>3)M.S[0]%=3+0*e(ei,10,M.S[1],d[0].S[1])*e(es,6,M.S[1+s],M.S[8+s]);
m;
}}